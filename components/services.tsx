...
export default function Services() {
  ...
  const services = [
    ...
    {
      icon: <Figma className="h-10 w-10 text-rose-500" />,
      title: "UI/UX Design",
        ...
        >
          {services.map((service, index) => (
            <motion.div key={index} variants={itemVariants}>
              <Card className="bg-gray-800/40 backdrop-blur-sm border-0 overflow-hidden hover:shadow-lg hover:shadow-rose-500/20 transition-all duration-300 h-full">
                <div className={`h-2 bg-gradient-to-r ${service.color}`}></div>
                <CardContent className="p-6">
                  <div className="w-16 h-16 rounded-full bg-rose-500/10 flex items-center justify-center mb-6 mx-auto group-hover:bg-rose-500/20 transition-all duration-300 shadow-[0_0_15px_rgba(225,29,72,0.3)]">
                    {service.icon}
                  </div>
